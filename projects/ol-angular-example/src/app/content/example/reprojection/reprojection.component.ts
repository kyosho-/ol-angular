...
@Component({
  selector: 'app-reprojection',
  templateUrl: './reprojection.component.html',
  styleUrls: ['./reprojection.component.css']
})
export class ReprojectionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const resolutionsArray = new Array(22);
    for (let i = 0, ii = resolutionsArray.length; i < ii; ++i) {
      resolutionsArray[i] = startResolution / Math.pow(2, i);
    }
  ...
  updateViewProjection(event: Event) {
    const value = (event.target as HTMLSelectElement).value;
    const newProj = getProjection(value);
  ...
  updateRenderEdgesOnLayer(layer: Layer) {
  ...
  changeBaseLayer(event: Event) {
    const value = (event.target as HTMLSelectElement).value;
    const layer = this.layers[value];
    if (layer) {
      layer.setOpacity(1);
  ...
  changeOverlayLayer(event: Event) {
    const value = (event.target as HTMLSelectElement).value;
    const layer = this.layers[value];
    if (layer) {
